...
export default function Leaderboard() {
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-700 dark:text-gray-300 animate-pulse">{t('loading')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black">
      ...
      {/* Header */}
      <header className="relative pt-24 border-b border-gray-300 dark:border-zinc-800/50 backdrop-blur-xl bg-gray-100 dark:bg-zinc-900/50">
        <div className="container mx-auto px-4 py-8">
          <div className="text-center mb-6">
            <span className="inline-block px-4 py-2 bg-orange-500/30 dark:bg-orange-500/20 border border-orange-500/50 rounded-full text-orange-600 dark:text-orange-300 text-sm font-semibold mb-4">
              {t('badge')}
            </span>
            <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-orange-500 via-red-600 to-orange-600 bg-clip-text text-transparent mb-2">
              {t('title')}
            </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6 text-xs sm:text-sm">
            <span className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
              ...
              <span className="text-base sm:text-lg font-semibold text-orange-500">{totalVotes}</span> {t('totalVotes')}
            </span>
            ...
            <span className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
              ...
              <span className="text-base sm:text-lg font-semibold text-red-500">{t('topFive')}</span> {t('publicLeaderboard')}
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="relative container mx-auto px-4 py-6">
        <Link
          href="/"
          className="inline-flex items-center gap-2 px-6 py-3 bg-gray-200 dark:bg-zinc-900 hover:bg-gray-300 dark:hover:bg-zinc-800 border border-gray-300 dark:border-zinc-800 hover:border-orange-500/50 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white rounded-xl transition-all duration-300"
        >
      ...
      <div className="relative container mx-auto px-4 pb-12">
        <div className="bg-gray-100 dark:bg-zinc-900 backdrop-blur-xl rounded-2xl overflow-hidden border border-gray-300 dark:border-zinc-800 shadow-2xl">
          {topSongs.length === 0 ? (
            <div className="p-16 text-center">
              <svg className="w-20 h-20 mx-auto text-gray-400 dark:text-zinc-700 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M12 12h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-gray-600 dark:text-gray-400 text-lg">{t('noVotes')}</p>
              <p className="text-gray-400 dark:text-gray-600 text-sm mt-2">{t('beFirst')}</p>
            </div>
          ) : (
            <>
            <div className="bg-gradient-to-r from-orange-900/20 to-red-900/20 p-6 border-b border-orange-500/30 dark:border-orange-500/20">
              <div className="flex items-center justify-center gap-3">
                <svg className="w-6 h-6 text-orange-500" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                </svg>
                <p className="text-gray-700 dark:text-gray-300 text-center">
                  <span className="font-semibold text-gray-900 dark:text-white">{t('topFiveRankings')}</span> - {t('fullAnalytics')}
                </p>
              </div>
            </div>
            <div>
            {topSongs.map((song, index) => {
              ...
              const isTopThree = index < 3;
                ...
                return (
                  <div
                    key={song.id}
                    className={`group relative flex items-center gap-2 sm:gap-4 p-3 sm:p-6 border-b border-gray-300 dark:border-zinc-800 last:border-b-0 hover:bg-gray-200 dark:hover:bg-zinc-800/50 transition-all duration-300 ${
                  ...
                  >
                    ...
                    {/* Rank */}
                    <div className="relative flex-shrink-0 w-10 sm:w-16 text-center">
                      {index < 3 ? (
                        ...
                        <div className="text-xl sm:text-2xl font-bold text-zinc-600">
                          #{index + 1}
                        </div>
                      )}
                    </div>
                    ...
                    {/* Album Art */}
                    {song.albumImage && (
                      <div className="relative flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded-lg overflow-hidden shadow-lg group-hover:shadow-orange-500/30 transition-shadow">
                        <Image
                          src={song.albumImage}
                          alt={song.albumName}
                          fill
                    ...
                    <div className="relative flex-1 min-w-0">
                      <h3 className="font-bold text-sm sm:text-lg text-gray-900 dark:text-white truncate mb-1 group-hover:text-orange-400 transition-colors">
                        {song.name}
                      </h3>
                      ...
                      <div className="mt-2 sm:mt-3 hidden sm:block">
                        <div className="flex items-center gap-3 mb-1">
                          <div className="flex-1 h-2 bg-gray-300 dark:bg-zinc-800 rounded-full overflow-hidden">
                            <div
                          ...
                          <span className="text-xs sm:text-sm font-medium text-gray-400 min-w-[3rem] text-right">
                    ...
                    {/* Vote Count */}
                    <div className="relative flex-shrink-0 text-center px-2 sm:px-6">
